...
foreach ($pageIndexes as $category => $products) {
    ...
    for ($i = 0; $i < count($products); $i++) {
        ...
        $categoryPage = file_get_html('http://irtelecom.az/catalog/index/' . Data::getPageNum($products[$i]));
        //< Taking the page of the model
...
from marks
where mark_name='" . Data::getSubcategory($products[$i]) . "'),'" . addslashes(htmlspecialchars($prodColor)) . "','" . addslashes(htmlspecialchars($prodDescr)) . "','" . (0.5 * rand(0,
                        10)) . "','" . addslashes(htmlspecialchars($prodOperSys)) . "')";
            if ($category != end($pageIndexes) || $i < count($products) - 1 || $j < count($aTags) - 1) {
                $sql .= ',';
            }
            //< Creating final query
        }
