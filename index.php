...
//> Necessary pages
$pageIndexes = Data::getCategories();
//< Necessary pages
...
$counter = 0;
foreach ($pageIndexes as $category => $subCategory) {
    $counter++;
    for ($i = 0; $i < count($subCategory); $i++) {

        //> Taking the page of the model
        $categoryPage = file_get_html('http://irtelecom.az/catalog/index/' . Data::getLabel($subCategory[$i]));
        //< Taking the page of the model
        ...
        for ($j = 0; $j < count($divTags); $j++) {
            ...
            if (!file_exists('images/' . basename($imgSrc))) {
                copy($imgSrc, 'images/' . basename($imgSrc));
            }
            //< Taking the link of the image of the product and saving it
            ...
            if (!empty($prodOperSys) && strtolower($prodOperSys[0]) == 'i') {
                $prodOperSys = "iOS";
            } else {
                $prodOperSys = "Android";
            }
...
from marks
where mark_name='" . Data::getPageNum($subCategory[$i]) . "'),'" . addslashes(htmlspecialchars($prodColor)) . "','" . addslashes(htmlspecialchars($prodDescr)) . "','" . (0.5 * rand(0,
                        10)) . "','" . addslashes(htmlspecialchars($prodOperSys)) . "')";
            if ($counter < count($pageIndexes) || $i < count($subCategory) - 1 || $j < count($divTags) - 1) {
                $sql .= ',';
            }
            //< Creating final query
        }
